...
function loadSofa(index) {
    ...
    loader.load(
        ...
        (xhr) => {
            const percent = Math.floor((xhr.loaded / (xhr.total || 3000000)) * 100); // 假设文件约3MB
        ...
        (error) => {
            ...
            progressText.textContent = '加载失败，请检查控制台';
            setTimeout(() => {
                loadingIndicator.style.display = 'none';
            }, 2000);
        }
...
function initHandleSelector() {
    const defaultHandle = 'Golden';
    selectHandle(defaultHandle);
    document.querySelector(`[onclick="selectHandle('${defaultHandle}')"]`)
...
function selectHandle(handleType) {
    ...
    const loader = new THREE.GLTFLoader();
    loader.setDRACOLoader(dracoLoader); // 添加这行代码，使用全局的dracoLoader
    
    loader.load(handleModels[handleType], (gltf) => {
        console.log('拉手模型加载成功:', gltf);
        ...
        document.querySelector(`[onclick="selectHandle('${handleType}')"]`)
            .classList.add('selected');
    }, undefined, (error) => {
