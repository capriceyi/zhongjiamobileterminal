...
// 配置 GitHub 仓库信息
const GITHUB_USER = 'capriceyi';
const REPO_NAME = 'my-sofa-project';
const BRANCH = 'main'; // 或您使用的分支名
const BASE_URL = `https://cdn.jsdelivr.net/gh/${GITHUB_USER}/${REPO_NAME}@${BRANCH}/`;
...
function loadSofa(index) {
    if (index < 0 || index >= SOFA_MODELS.length) {
    ...
    const modelUrl = BASE_URL + SOFA_MODELS[index];

    // 显示加载指示器
    loadingIndicator.style.display = 'flex';
    progressText.textContent = window.innerWidth <= 768 ? '加载中，请稍候...' : '0%';
    ...
    const timeout = setTimeout(() => {
        ...
        console.error('加载超时，尝试加载的URL:', modelUrl); // 添加这行
    }, 60000); // 从10000改为30000
    ...
    loader.load(
        ...
        (xhr) => {
            ...
            const percent = xhr.total === 0 ? 
                Math.min(95, Math.floor(xhr.loaded / 3000000 * 100)) : 
                Math.floor(xhr.loaded / xhr.total * 100);
            
            progressBar.style.width = `${percent}%`;
            progressText.textContent = `${percent}%`;
            progressText.style.color = percent > 50 ? '#333' : '#fff';
        },
        (error) => {
            console.error('加载模型时发生错误:', error);
            console.log('尝试加载的URL:', modelUrl); // 添加这行
...
function selectHandle(handleType) {
    ...
    const loader = new THREE.GLTFLoader();
    loader.setDRACOLoader(dracoLoader);
    const modelUrl = BASE_URL + HANDLE_MODELS[handleType];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.handle-option').forEach(opt => {
        opt.addEventListener('click', function() {
            selectHandle(this.dataset.handle);
        });
    });
    ...
    document.querySelectorAll('.sofa-item').forEach(item => {
        item.addEventListener('click', function() {
            ...
            loadSofa(index);
            document.getElementById('sofa-name').textContent = this.querySelector('p').textContent;
